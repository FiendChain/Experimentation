...
set helplang=En
set laststatus=2
set runtimepath=~/vimfiles,~\\vimfiles\\plugged\\vim-gitgutter,~\\vimfiles\\plugged\\editorconfig-vim,~\\vimfiles\\plugged\\vim-airline,~\\vimfiles\\plugged\\vim-airline-themes,~\\.fzf,~\\vimfiles\\plugged\\fzf.vim,~\\vimfiles\\plugged\\emmet-vim,~\\vimfiles\\plugged\\nerdtree,~\\vimfiles\\plugged\\vim-multiple-cursors,~\\vimfiles\\plugged\\vim-eunuch,~\\vimfiles\\plugged\\vim-surround,~\\vimfiles\\plugged\\ale,~\\vimfiles\\plugged\\vim-monokai,~\\vimfiles\\plugged\\delimitMate,C:\\Program\ Files\ (x86)\\Vim/vimfiles,C:\\Program\ Files\ (x86)\\Vim\\vim81,C:\\Program\ Files\ (x86)\\Vim/vimfiles/after,~/vimfiles/after
set shiftwidth=4
set showtabline=2
...
silent only
cd ~\Dropbox\Coding\Languages\Java\Projects\Java\ Sandbox
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +1 Sandbox.java
badd +1 net\shape\Shape.java
badd +1 net\shape\Square.java
badd +1 net\exception\ValueError.java
argglobal
silent! argdel *
edit Sandbox.java
...
wincmd w
argglobal
if bufexists('net\shape\Square.java') | buffer net\shape\Square.java | else | edit net\shape\Square.java | endif
let s:cpo_save=&cpo
set cpo&vim
imap <buffer> <S-BS> <Plug>delimitMateS-BS
imap <buffer> <BS> <Plug>delimitMateBS
...
exe 'vert 1resize ' . ((&columns * 119 + 119) / 238)
exe 'vert 2resize ' . ((&columns * 118 + 119) / 238)
tabedit net\exception\ValueError.java
set splitbelow splitright
set nosplitbelow
set nosplitright
wincmd t
